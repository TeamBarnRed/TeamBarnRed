      ...
      static void Main(string[] argumentite)
      {
         ...
         while (!proverka(matrica))
         {
            ...
            if (вход == "top")
            {
               for (int i = 0; i < 5; i++)
               {
                  Console.WriteLine("Name : {0} , moves : {1} ", TopScores__.igrachi[i], TopScores__.hodove[i]);
               }
               continue;
         ...
         {  if (TopScores__.hodove[i] > moves)
            {
               Console.WriteLine("Congratulations, you have just putted a new record");
               Console.Write("Please enter your name : ");
          
             TopScores__.hodove[i] = moves ;
             
             TopScores__.igrachi[i] = Console.ReadLine();
            }
         }
